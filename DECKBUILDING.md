...
There is no pre-match Straight-vs-Lance choice.
...
- **Straight Shot is a universal baseline** — every player always has it, like
  ...
  player has drafted Lance.
- **Lance Shot is the first draftable ability**, not a starting pick. It lifts
...
entry 0; Lance (once drafted) appends as a fallback entry.

**Disjoint fallbacks vs. genuine conflicts.** The Straight/Lance relationship
is a *disjoint fallback*: they never contend for the same tile (Straight acts
from unclaimed ground, Lance only when Straight fizzles from owned ground), so
...
trigger. Border Grinder (#3) and Contested Ground (#15) are the first such pair —
...
Border Grinder/CG rule) and "a later-appended entry is a fallback" (Lance) coexist
...
(Straight/Lance's mode is decided by context, not a button). It doesn't fit
...
- **Tap** (release before a hold threshold, ~150-200ms TBD): a normal single
  Straight/Lance shot at 1 charge, unchanged from today.
...
current roster is either a passive always-on modifier (Border Grinder,
Splitter, Ricochet, Body Blocker, Impaler, the whole economy family) or
auto-triggered by context with no cost choice (Straight/Lance's mode,
...
no beam / no charge when firing from a claimed tile without Lance) are
...
context-decides-the-outcome philosophy as Straight/Lance (§2), not a
...
kit for one counter-shot (e.g. parry a Border Grinder-owner's shot and it
still instantly flips an enemy tile). This is the sharpest with aggressive
kits — **a Breach Aggression or Border Grinder player's own beam, if
...
- **Border Grinder (#3)**: a parried beam that keeps this effect can
...
10. **Salvage** — refund 1 charge whenever a beam despawns claiming nothing (Lance misses). · on_resolve(null) · enabler · Lance.
...
32. **Unparryable** — your outgoing beams can't be parried. Close to a necessary tech piece once parry ships (§2) rather than a nice-to-have — without it, an aggressive kit (Breach, Border Grinder) risks being turned against its own owner by a parry. Same counter-tech shape as Impaler countering Body Blocker: a defensive universal mechanic gets an attacker-side answer. · on_fire · enabler (denies the opponent's parry option entirely) · Breach Aggression, Chain Cannon, any archetype whose own beam effects would be dangerous if turned around.

### Zoning addendum (Barrier + Bulwark — grouped by role with Landmine #17 and the stack modifiers #23–27, not with the parry cluster above)
33. **Barrier** — on resolve, the beam's landing/resolve tile (that tile only — *not* Splitter's bonus neighbors) arms as a **one-shot knockback pad**: the first enemy who tries to walk onto it is repelled one tile back, reusing the existing `KnockbackEffect`/`IsKnockedBack` machinery (§6), after which the pad is spent and the tile reverts to an ordinary claimed tile (still owned, still dealing normal entry/standing damage). **Pure zoning, no damage** — deliberately distinct from Landmine (#17), which is pure damage on a similar owned-tile trigger; the two **compose** (barriers funnel the enemy's path, Landmine/hostile-tile attrition punishes where they're funneled). **Deliberate placement with no new input (§2)**: the "activation" is your normal shot, so every barrier costs a charge and lands exactly where you aimed — a payoff in the **Splitter (#4) family** (Splitter claims the landing tile's neighbors; Barrier arms the landing tile itself), *not* the arm-on-neglect Landmine family (§2 forbids a dedicated key, and long-press is already double-booked by Full Draw + Wide Shot, §8's "strongest hold-coupling"). **One-shot-and-revert is the load-bearing limiter**: it makes each barrier a *speed bump at your frontier*, not a permanent fortress, so a Solar-Economy-fed player can't wall the whole board — i.e. it's the drafted, bounded form of the rejected global knockback-on-entry mechanic (which would have hollowed the HP vector, §8, and forced stalemate). Reads as an **Iron Wall (C) zoning payoff**. **Interactions**: beams still pass through territory, so Border Grinder (#3)/Contested Ground (#15) can flip a barrier tile and destroy it — the intended counter, but it lands on the *movement* axis rather than the beam-walling axis §4's C-beats-B leg is built on, so flag for the balance pass (§7) whether it distorts that leg (unverified until playable, same discipline as §8). `on_body_hit` drag (§1) can shove an enemy *into* a barrier, or a barrier can halt a drag crossing the shooter's own territory — a tactical combo that needs an explicit per-tick ordering rule (which `KnockbackEffect` wins). Knockback into a map edge or a second barrier no-ops the bounce (`apply_knockback` requires valid ground) and pins the enemy in place — a punishing corner, not a bug. Shared-screen visibility (§17 reasoning): armed pads carry a subtle `*EffectTarget` marker — routable when the enemy walks, unavoidable when dragged. **Full Draw (#12)**: with both drafted, each tile in Full Draw's extended claim also arms as a one-shot barrier, laying a *line* of pads down the lane — beam-line tiles only (the resolve tile + Full Draw's line extension); Splitter's perpendicular bonus claims still never arm, consistent with the base "that tile only" rule above. This *reach* axis is orthogonal to Bulwark's (#34) per-tile *durability* axis, so the two compose (long wall × tough tiles) without contending, provided durability is never routed through Full Draw's own budget (that would be the §2 resource conflict — see #34). · on_resolve · payoff (zoning/denial) · Landmine (funnel-and-punish), Solar Economy (de facto enabler — more claimed tiles = more shots that can drop pads), Splitter.
34. **Bulwark** — stack modifier for Barrier (#33); does nothing without it drafted. Raises Barrier's durability cap by +1 hit per copy (N = 1 + copies): instead of being spent by the first enemy it repels, an armed barrier stays standing and **drains 1 charge from the owner's live charge reserve on each subsequent hit** to repel again, falling only once N hits have been absorbed **or** the reserve runs dry — whichever comes first (this is what "falls only after N provisioned charges" means, once you separate durability from Full Draw — see below). **Deliberately erodes Barrier's load-bearing safety property** (§33's one-shot-and-revert = speed bump, not fortress): a durable barrier trends toward the drafted-stalemate/boxing failure mode the one-shot rule exists to prevent, so Bulwark is only balanced because it carries its own limiter — **the live charge drain itself**. A wall under sustained assault burns the owner's charge income; every charge feeding the wall is one not spent attacking or expanding, so a player cannot turtle and push at once (same self-limiting economy logic as Breach's per-charge chain cost, #16). **Cross-archetype tie**: the wall's ammo is the charge reserve, so Solar Panels (#9) directly fuels barrier durability — an A(economy)-feeds-C(wall) thread — in natural tension with Battery Cap (#14): reservoir-to-dump vs. steady-flow-to-feed-the-wall. **Flip counter unchanged, and now sharper**: Border Grinder (#3)/Contested Ground (#15) flip the barrier tile and erase it regardless of remaining hits, so a charge-expensive Bulwarked wall still dies to a single one-charge flip — a clean cost asymmetry that *deepens* #33's C-beats-B RPS-leg flag (Bulwark makes C invest hard in walls B hard-counters); flag as a sharper balance hotspot than base Barrier (§7). **Distinct axis from Full Draw (#12)**: Bulwark owns *durability* (toughness-per-tile), Full Draw owns *reach* (how many barrier tiles — see #33's Full Draw note); they read orthogonal axes and compose the same clean way Full Draw + Wide Shot do (§8) — Full Draw lays a line of barriers, Bulwark makes each tile of it N-tough. Do **not** route durability through Full Draw's own budget (both would consume the same magnitude — a genuine resource conflict, §2); keeping the axes separate is exactly what lets them compose. No input rework — fully passive (Barrier's on_resolve placement + a drain-on-hit resolver). Soft cap on N recommended (TBD, balance pass §7), same category as Bank Shot (#23)/Juggernaut (#27). · on barrier-hit (drain a charge + persist, as the barrier's owner) · enabler (extends Barrier's durability; same "does nothing alone" stack-modifier shape as Bank Shot/Capacitor/Juggernaut) · Barrier (#33, required), Solar Panels (#9, feeds the drain), Full Draw (#12, orthogonal reach axis), anti-synergy-ish with Battery Cap (#14).
...
nothing* (Border Grinder vs. Contested Ground before its resolution rule,
...
roster is deliberately light here: essentially Border Grinder/CG and Battery
Cap/Solar Panels.
...
both Border Grinder and Contested Ground are in play, show *"Border Grinder
...
**Shop UX decision (Phase 2): a shared competitive grab-race room**
(Binding-of-Isaac-style). Between rounds, both characters spawn into a special
walkable room where drafted/consumable items sit as world pickups; **the first
player to walk onto an item claims it** (first-come, not a symmetric pick-1-of-3
menu). This is a real-time contest *for the draft itself* and is inherently
asymmetric, so it belongs here in Phase 2, not the Phase-1 symmetric draft.
Two flags for whoever builds it: (a) both characters share one zoomed/panned
world camera (§ camera notes / `CLAUDE.md`), so item spawns must stay inside the
barycenter framing; (b) grab-race is swingy — consider per-item claim cooldowns
or a one-pick-each cap so the faster player can't sweep the whole room. The
walk-to-pick room is just a different *acquisition UX* over the same descriptor
data model — no change to how abilities are represented or resolved.
...
behaviors and charge modifiers are data descriptors read by resolver systems.
The systems are split by responsibility: spawn-time beam-behavior selection
(e.g. picking Lance) lives in the **beam** plugin; the `on_resolve`/`on_claim`
...
charge modifiers (regen/refund/cost policy) are read by resolvers in the
**charge** plugin, the owner of `BeamCharges`; `on_body_hit`-side modifiers
(Body Blocker) stay in the **damage** plugin, the owner of `Health`/knockback.
Reserve component-bundles for abilities needing bespoke query shapes
(Landmine's per-tile timers, Beachhead's radius burst).
...
contend for the same tile/trigger (§2 — first is Border Grinder vs. Contested
...
2D camera on a new `OVERLAY_RENDER_LAYER`, ordered above the HUD camera, with
`ClearColorConfig::None`) so it never pans/zooms with the world camera or gets
clipped to the HUD's top strip. Text is composed from a **bitmap-font glyph
atlas** (`assets/font.png`) via a `spawn_label(&str)` helper that spawns one
sprite per glyph — reusing the project's global nearest filtering and
`bevy_tweening` for SF2-style pop/scale. The same overlay + glyph machinery
serves both the "3 · 2 · 1 · GO!" intro countdown and the eventual win banner
(and later, shop price labels like "5¢"). Note glyph sprites must carry
`RenderLayers` per-entity — layers don't inherit through the hierarchy without
`Propagate`.
...
- **Border Grinder is B's early scaffold** (see the caveats at the end) — it
...
payoff depends on that judgment (Border Grinder — only pays off if you aim at
an enemy-owned tile instead of an unclaimed one; `src/plugins/bot.rs`'s
`border_grinder_target`/`effective_reach` exist specifically to teach the bot this).
This is orthogonal to "passive" (§2/§3's sense: no separate activation
input) — Border Grinder is passive *and* decision-bearing at once. A bot's
...
2. **Kit vs. Straight-only → absolute power** ("does this kit earn its slots?").
   ...
   exercised here as part of B: `{BorderGrinder, Lance}` vs. `{}`.
   **Mode: bot-vs-bot is trustworthy for decision-free abilities** (Solar
   Panels). **For decision-bearing abilities (Border Grinder and beyond),
...
balancing it rides on B's kit alone (in the scaffold, optionally `{BorderGrinder}` or
`{BorderGrinder, Lance}`), measured via layers 2 and 4, never smuggled onto the A
...
- *architecture (§6):* replaced `Beam.inverted: bool` with a two-part split:
  - `BeamBehavior { Straight, Lance }` — the *per-beam, transient* resolved
    execution mode (`src/components/beam.rs`). `spawn_beam` always selects
    `Straight` in F1; the `Lance` arm holds the **ported ex-inverted logic**
...
- *decisions taken during implementation:*
  1. **"Final Straight now"** — inverted is *inactive* in F1 (Straight fizzles
     from owned ground); it returns in F2 as the drafted Lance ability. So F1
  ...
  2. **Charge spent on fire, not on resolve (§2:60-62)** — a fizzle now costs a
     ...
     **Superseded**: firing from a claimed tile without Lance is now gated at
     ...
     Charge-on-fire remains the mechanism for beams that do fire, e.g. a Lance
...
**Stage F2 — descriptor container + Lance (plumbing proof)** — **IMPLEMENTED**.
...
- *decisions taken during implementation:*
  1. **Contextual selection, not wholesale mapping** — Lance is a *fallback*
     (§2): `spawn_beam` selects `BeamBehavior::Lance` only when the beam is
     fired from already-claimed ground (where Straight would fizzle) **and** the
     owner has drafted `Lance`. This reproduces the exact pre-F1 auto-selected
  ...
  2. **`on_resolve` resolver deferred to Slice 1; `on_claim` deferred further**
     ...
     Slice 1's real consumer is Border Grinder (`on_resolve`); that resolver
...
- *architecture (§6):* round segmentation implemented as the project's **first
  ...
  countdown (gameplay frozen), `Playing` is live, `Outcome` shows the win
  banner; a full reset loops `Outcome → Starting`. Live-gameplay systems
  (input, movement, beam spawn/step, charge spend, damage) gate on
  `in_state(Playing)` — the intro and win screens freeze the world for free.
  This implements the two-vector round-ending model (§5): **kill (HP→0,
  instant/highest priority)** + **timeout resolving by tile-count-then-HP**,
  *both live from this stage — not kill-only.* First-to-N-tiles is optional here
  (same vector as timeout-by-tiles); add it when the tile race wants an explicit
  finish line. Round boundary is a full reset of board + charges (§1/§5); include
  the Beachhead reset-exception hook now, even though Beachhead itself lands in
  Heavy content below. Round-phase text renders via the screen-space overlay
  camera + glyph banner described in §6 "Visuals".
- *rollout within F3a:* the **intro-countdown slice ships first** — it exercises
  the full presentation stack (state machine, overlay camera, glyph banner,
  one-shot scale tween) with **no win-resolution logic**, giving a visible
  "gameplay frozen → 3 · 2 · 1 · GO! → control unlocks" before the win/timeout
  resolution and reset loop are built. The win banner then reuses 100% of it.
- *abilities:* none.
...
- *architecture (§6):* `on_charge_regen` tick (Solar Panels); `on_resolve`
  enemy-owned-tile flip resolver (Border Grinder). No `on_claim` hook this
...
- *abilities:* A = Solar Panels (#9); B = Border Grinder (#3) scaffold
...
- **Contested tiles + Breach**: the heavy data-model change —
  ...
  the Border Grinder scaffold to its canonical deep-strike kit.** The
  Border Grinder/Contested Ground conflict resolver (§2/§6) ships here, since
...
- **Capstones + area**: Impaler (#8, guaranteed-Rare offer,
  persistent-layer only), Landmine (#17), Barrier (#33, reuses Landmine's
  small component-bundle shape + the existing `KnockbackEffect` machinery, on an
  `on_resolve` payoff resolver), Bulwark (#34, Barrier's durability stack modifier — a drain-on-hit resolver reading the owner's charge reserve), Beachhead (#19, uses the F3a
...
- **Matchup balance necessarily lags each real kit.** Border Grinder-as-B lets you
  ...
  canonical B matchup or the RPS triangle. Border Grinder is incremental border-grind;
...
- **Border Grinder fills a genuine gap, doesn't override the plan.** Border Grinder had
...
- **Consumables/enhancers layer.** Balatro separates permanent build pieces
  ...
  whether a consumable layer is needed on top. If a consumable layer does land,
  the shared competitive grab-race room (§5 Phase 2) is its natural delivery
  mechanism — consumables as world pickups the players race for, distinct from
  the between-round ability draft.
...
- **Direct-HP payoffs are almost entirely unexplored.** Riposte (#28) is
  ...
  **The real deferral risk — over-fitting the round knobs during the archetype
  ...
  spirit*", and Border Grinder-as-B "does not validate the canonical B matchup", §7).
