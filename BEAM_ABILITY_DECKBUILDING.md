...
explicit activation. Overcharge and Wide Shot are the only two that qualify
today.
...
beam (`on_body_hit`) while mid-hold on Overcharge or Wide Shot, the hold is
...
- **Wide Shot (#6) — parry is per-lane, never re-widens.** Wide Shot spawns
  ...
  bounce budget, an Overcharge claim budget) but does not re-trigger Wide
...
15. **Contested Ground** — by default, flipping an enemy-claimed tile is impossible in any form (§1); this ability introduces that capability, but conditionally. Trigger is the *first enemy-owned tile* a beam's path would otherwise stop at (Straight) or skip past (Pierce) — mandatory when it fires, decided entirely by where you aim, no separate input. Instead of blocking/skipping, the beam resolves there and starts a 2s countdown; if you still **own** a tile adjacent to it when the countdown ends, it flips to you, otherwise nothing happens and the charge is spent for no gain. **Ownership check, not player position**: this is a `ClaimedTile` lookup at the 2s mark, not a requirement to stand nearby — the attacker can walk away and fire other Straight/Pierce shots freely the moment the contest starts, and it resolves independently in the background based on board state. No interference with normal shot behavior. While pending, the tile is neutral for *other* beams' travel. **Intentionally weak/risky as a standalone pick** — Overpenetration (#3) does the same flip instantly and unconditionally, so nobody should draft this for the flip itself. Its real job is the pending-neutral travel window, which is the only thing Breach can hook into (Overpenetration's flip is instant and solid, creating no window) — this is a pure enabler that's a trap pick without Breach, by design. **Interactions**: Splitter triggers on the contest's start (claims its 2 orthogonal neighbors if unclaimed, giving a small guaranteed floor even if the contest later fails); Chain Reaction triggers on the eventual flip like any other claim; Ricochet just passes through a pending tile rather than turning at it; Overcharge chains through multiple enemy tiles at once if Breach is also drafted (see #12). **Anti-synergy with Overpenetration (#3)**: drafting both is likely wasted value, not a combo — Overpenetration's instant unconditional flip would take priority the moment a beam reaches an enemy tile, so Contested Ground's contest never gets a chance to trigger for that player. They're alternative answers to the same problem for different playstyles, not meant to be stacked. See Overrun (#27) for a cheaper single-shot way to open multiple contests at once, as a complement to Breach's multi-beam chain rather than a replacement for it. · on_resolve (first enemy-owned tile encountered) · enabler (Breach and/or Overrun; weak alone) · Breach, Overrun.
...
27. **Overrun** — +1 additional consecutive enemy-owned tile affected per copy drafted, extending whichever of Overpenetration (#3) or Contested Ground (#15) the drafting player has — a single shared parameter across the pair, same "shared resource across multiple abilities" shape as Fallout (#26), not Bank Shot/Capacitor's single-target scaling. Does nothing without at least one of the two drafted. **With Overpenetration**: instead of resolving on the first enemy-owned tile and stopping, the beam keeps traveling and instantly overwrites each of the next N consecutive enemy-owned tiles in the line (N = 1 + copies drafted), reverting to normal resolve/stop behavior the instant it reaches a tile that isn't enemy-owned (neutral, self-owned, forbidden) or the budget runs out. Each flip in the run is a real `on_resolve` event — Splitter, Tithe, Chain Reaction all fire per tile in the run, same principle as Ricochet's bounces (#5), not a free pass-through. **With Contested Ground**: instead of the beam being consumed opening a single contest, it keeps traveling and opens up to N simultaneous independent contests across the next N consecutive enemy-owned tiles in the same pass — each with its own 2s countdown and its own adjacent-owned-tile check at expiry. One beam, one charge, reaching multiple tiles deep without Breach's (#16) per-tile follow-up-beam cost. Breach still chains further beyond wherever the run ends, so the two compound rather than compete — Overrun buys reach for free within a single shot, Breach buys further reach at further cost once that shot's run is spent. **Anti-synergy carries over, doesn't create a new one**: if both Overpenetration and Contested Ground are drafted, Overpenetration's per-tile priority (see #3) still applies first — Overrun just extends whichever of the two is actually resolving each tile, it doesn't add a third interaction. Soft cap TBD, session 6 balance pass — same combo-risk category as Ricochet+Splitter and Fallout+Splitter/Chain Reaction (every additional tile in the run is a fresh claim/contest event, multiplying per copy) — though naturally self-limiting the way Chain Reaction is: a run only extends as far as contiguous enemy territory actually exists in that line, a finite, board-state-dependent resource, not an unconditionally growing one. · on_resolve/on_step (extends how many consecutive enemy tiles one shot affects) · enabler (shared across two payoffs, same shape as Fallout) · Overpenetration, Contested Ground (either), Breach (compounds beyond the run), Splitter/Chain Reaction (combo-risk multiplier), Breach Aggression and Chain Cannon archetypes.
...
- **A — Solar Economy** (Solar Panels + Tithe + Frugal Frontier → Overcharge/Chain Reaction, or → Reckoning as an alternate win-condition capstone): wide-claim engine, snowballs regen, dumps into board-flooding or cashes the board out directly via Reckoning. Overcharge is a hold-to-charge payoff (§3) that reads the lane in real time rather than a blind flat-cost gamble, and Capacitor cheapens it further — with Splitter also drafted, this is a genuine cross-archetype bridge into Chain Cannon (D), not just an economy payoff. Weak to early aggression.
