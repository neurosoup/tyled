 ...
 *               the `Countdown` timer, round resolution (kill/timeout) and the
 *               full round reset. Gameplay systems elsewhere gate on
 *               `in_state(RoundPhase::Playing)`.
 *   `intro`   — the round-start "3 · 2 · 1 · GO!" banner shown during `Starting`.
 *   `outcome` — the win banner shown during `Outcome`, which loops the round back
 *               to `Starting` after the reset.
 *
