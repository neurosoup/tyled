...
Every stage of the gameplay pipeline declared in `schedule.rs`'s `GameplaySet` chain gates on `Playing`, with two exceptions (see Gaps below):
...
- These two are not the same kind of thing as the deliberately-ungated systems elsewhere:
  ...
  - `effects.rs` is almost entirely ungated (only `clear_illumination_drivers` is hooked, to `OnExit(Playing)`) — consistent with tweens (bounce, knockback, death animation) needing to keep playing across the `Playing → Outcome` boundary, most visibly the death bounce that plays out after a kill ends the round.
...
subgraph pipeline ["Playing-gated gameplay pipeline (GameplaySet chain)"]
  bot_plugin
  inputs_plugin
  controller_plugin
  beam_plugin
