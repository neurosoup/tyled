...
- `AbilityDescriptor` (`src/components/abilities.rs`) — a single draftable ability, kept as pure, `Reflect`-serialisable data (no `Entity` or runtime handles) so a future loadout can be authored in RON, hot-reloaded via `file_watcher`, and persisted across sessions. Declares one variant, `Lance`.
- `PlayerLoadouts` (`src/components/abilities.rs`) — a **resource** holding the hardcoded P1/P2 starting descriptor lists (there is no draft UI). Inserted by this plugin's build — currently **empty for P1 (Straight-only) and `[Lance]` for P2** — and read by `initialize_players` via `for_player(player_id)`. It is the single place to assign or swap kits between runs; adding `AbilityDescriptor::Lance` to a player's list grants that player the ability.
...
None. The plugin only registers reflected component types and inserts the `PlayerLoadouts` resource. Lance needs no claim-side resolver — only spawn-side behavior selection in the Beam plugin.
...
- `AbilityDescriptor` — `#[derive(Reflect, Clone, Debug, PartialEq, Eq)]`, one variant `Lance`.
